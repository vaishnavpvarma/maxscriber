    ...
    def insert_extractions_bulk(self, extractions: list):
        with sqlite3.connect(self.db_path) as conn:
            ...
            for file_name, meta, test_data in extractions:
                ...
                if age and age != 'nil':
                    cursor.execute('UPDATE patients SET age = ? WHERE max_id = ? AND (age IS NULL OR age = "nil")', (age, max_id))
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            for fname, grp in file_groups.items():
                ...
                for enc_id, cdate in grp['enc_ids'].items():
                    cursor.execute('SELECT test_name, value FROM lab_results WHERE encounter_id = ?', (enc_id,))
