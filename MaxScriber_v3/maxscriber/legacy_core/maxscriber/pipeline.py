...
from maxscriber.stratification import perform_stratification, run_kruskal_wallis, generate_pzfx
...
def setup_logging(output_dir: Path, job_name: str, append: bool = False, verbose: bool = False) -> logging.Logger:
...
def run_transcribe(input_dir: Path, output_dir: Path, job_name: str, verbose: bool = False, threads: int = os.cpu_count()):
    """
    ...
    if not verbose:
        with alive_bar(len(pdf_files), title='Transcribing PDFs', force_tty=True) as bar:
            with concurrent.futures.ProcessPoolExecutor(max_workers=threads) as executor:
                futures = {executor.submit(_process_pdf_worker, pdf, output_dir, job_name, verbose): pdf for pdf in pdf_files}
        ...
        for idx, pdf in enumerate(pdf_files, 1):
            pdf_name, content_hash, meta, final_test_data = _process_pdf_worker(pdf, output_dir, job_name, verbose)
            if not content_hash:
                failed_files.append(pdf_name)
            else:
                if content_hash in seen_hashes:
...
def run_plot(output_dir: Path, job_name: str):
    ...
    if do_strat == 'y':
        ...
        # Plot preference
        plot_type = input("\nWhich plot do you prefer for visualization? (1) Box-and-Whisker (2) Violin Plot: ").strip()
...
def run_all_phase1(input_dir: Path, output_dir: Path, job_name: str, verbose: bool = False, threads: int = os.cpu_count()):
    """
