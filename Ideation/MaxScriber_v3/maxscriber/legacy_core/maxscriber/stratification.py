...
def generate_pzfx(stratified_data, output_path: Path, job_name: str, plot_type: str):
    ...
    ET.SubElement(created, f'{{{NS}}}OriginalVersion', {
    ...
    for i, var in enumerate(stratified_data.keys()):
        ET.SubElement(table_seq, f'{{{NS}}}Ref', {'ID': f'Table{i}', 'Selected': '1' if i == 0 else '0'})

    # 4. Tables
    for i, (var, groups) in enumerate(stratified_data.items()):
        table = ET.SubElement(root, f'{{{NS}}}Table', {
            'ID': f'Table{i}', 
            'XFormat': 'none', 
            'TableType': 'OneWay',
        ...
        for group_name in ['Mild', 'Moderate', 'Clinically Severe']:
            ycol = ET.SubElement(table, f'{{{NS}}}YColumn', {'Width': '114', 'Decimals': '2', 'Subcolumns': '1'})
