...
def main():
    ...
    try:
        if args.command == 'run':
            ...
            run_all_phase1(inp, out)

            # The Gate: conditional verification
            ...
            if should_continue:
                ...
                logger.info(f"extraction.log preserved at {out / 'extraction.log'}")
            ...
            run_transcribe(Path(args.input_dir), Path(args.output_dir))

        elif args.command == 'qc':
            run_qc(Path(args.output_dir))

        elif args.command == 'stats':
            run_stats(Path(args.output_dir))

        elif args.command == 'plot':
            run_plot(Path(args.output_dir))
