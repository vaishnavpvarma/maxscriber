...
def generate_condensed_stats(
    ...
    for n_files in sorted(file_count_groups.keys(), reverse=True):
        ...
        stats.append(f"{cat_str.ljust(col_cat)}| {str(n_pts).ljust(col_pts)}| {calc_str.ljust(col_calc)}| {str(n_pdfs).ljust(col_pdfs)}")
    stats.append(sep_line)
    if table_total_pts > 0:
        stats.append(f"{'TOTAL'.ljust(col_cat)}| {str(table_total_pts).ljust(col_pts)}| {''.ljust(col_calc)}| {str(table_total_pdfs).ljust(col_pdfs)}")
    else:
    ...
    if ages:
        ...
        stats.append(f"Youngest Patient: {min_age} Years (Source File: {', '.join(youngest_files)})")
        stats.append(f"Oldest Patient: {max_age} Years (Source File: {', '.join(oldest_files)})")
        stats.append("Age Groups:")
        stats.append(f"Adults (19–60 years): {adults} patients ({(adults / total_ages * 100):.1f}%)")
        stats.append(f"Children & Adolescents (0–18 years): {children} patients ({(children / total_ages * 100):.1f}%)")
        stats.append(f"Seniors (>60 years): {seniors} patients ({(seniors / total_ages * 100):.1f}%)")
    else:
    ...
    if all_dates:
        ...
        months = span.days / 30
        stats.append(f"Reporting Period: {min_date.strftime('%d %B %Y')} – {max_date.strftime('%d %B %Y')}")
    ...
    stats.append(f"         Unique Files (Remaining)   = {total_processed} - {n_dupes} - {n_failed} = {unique_files}")
    stats.append(f"Stage 3: Unique Patients (MAX_IDs)  = {actual_unique}")
    stats.append(f"Stage 4: Extra PDFs (Multi-file)    = {unique_files} - {actual_unique} = {extra_pdfs}")
    stats.append("")
    stats.append("Proof:")
    stats.append(f"  Total PDFs = Unique Patients + Extra PDFs + Duplicates + Failed")
    stats.append(f"  {total_processed} = {actual_unique} + {extra_pdfs} + {n_dupes} + {n_failed} = {reconstructed}")
    ...
    if reconstructed == total_processed:
        ...
        drift_ids = []
        for mid, fnames in file_name_map.items():
            overlap = fnames & duplicate_filenames if isinstance(fnames, set) else set(fnames) & duplicate_filenames
            if overlap:
