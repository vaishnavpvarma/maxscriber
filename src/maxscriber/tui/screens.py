...
from maxscriber.tui.modals import (
    ...
    SchemaWizardModal,
    WorkspaceLoaderModal,
)
from maxscriber.tui.widgets import ActionMenu, Banner, StatsPanel
...
class MainScreen(Screen):
    ...
    def _load_schema_registry(self):
        ...
        for s in schemas:
            ...
            status_txt = (
                Text("✅ [Active]", style="bold #33ff33")
                if is_legacy
                else Text("⚠️ [Review]", style="bold #ffcc00")
            )
    ...
    def _load_job_workspace(self, path: Path):
        ...
        try:
            ...
            cursor.execute(
                "SELECT name FROM sqlite_master WHERE type='table' AND name='longitudinal_data'"
            )
            if cursor.fetchone():
            ...
            for p in patients:
                ...
                id_txt = (
                    Text(f"🧬 [LONG] {pid}", style="bold #b142f5")
                    if pid in longitudinal_ids
                    else Text(pid, style="bold #e2e8f0")
                )
    ...
    def run_pipeline_worker(self, in_dir: Path, out_dir: Path):
        ...
        def progress_cb(pdf_name, confidence):
            ...
            self.app.call_from_thread(setattr, self.stats, "current_throughput", throughput)
            self.app.call_from_thread(setattr, self.stats, "current_confidence", confidence)

        # Run actual pipeline
        try:
            run_transcribe(
                in_dir, out_dir, threads=self.worker_threads, progress_callback=progress_cb
            )
