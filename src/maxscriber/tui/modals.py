...
class RecordInspectorModal(ModalScreen):
    ...
    def compose(self) -> ComposeResult:
        with Vertical(id="dialog", style="width: 80; height: 80%;"):
            ...
            syntax = Syntax(
                json_str, "json", theme="monokai", word_wrap=True, background_color="default"
            )

            yield Static(
                syntax, style="height: 1fr; overflow-y: auto; background: #1c2a35; padding: 1;"
            )
...
class ExecutePipelineModal(ModalScreen):
    ...
    def compose(self) -> ComposeResult:
        with Vertical(id="dialog"):
            ...
            yield Input(
                placeholder="Input Directory (e.g. ./raw_pdfs)", id="in-dir", classes="dialog-input"
            )
            yield Input(
                placeholder="Output Directory (e.g. ./output_run)",
                id="out-dir",
...
class SchemaWizardModal(ModalScreen):
    ...
    def compose(self) -> ComposeResult:
        with Vertical(id="dialog"):
            ...
            yield Input(
                placeholder="Schema Name (e.g. MY_LAB_V1)", id="sch-name", classes="dialog-input"
            )
            yield Input(
                placeholder="Assays (comma separated e.g. Hb, WBC, Platelets)",
                id="sch-assays",
                classes="dialog-input",
            )
