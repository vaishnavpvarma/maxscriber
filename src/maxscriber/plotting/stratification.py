...
def load_stratification_config(config_path: Path, logger: logging.Logger) -> dict:
    if config_path.exists():
        if not HAS_YAML:
            logger.warning(
                "PyYAML not installed. Cannot parse custom_stratification.yaml. Falling back to legacy Dengue mode."
            )
            return None
        try:
            ...
            logger.warning(
                f"Failed to load custom stratification config: {e}. Falling back to legacy Dengue mode."
            )
    return None
...
"""
    ...
    try:
        ...
        result = subprocess.run(
            ["Rscript", r_script_path.name], cwd=output_dir, capture_output=True, text=True
        )
